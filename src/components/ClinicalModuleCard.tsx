...
export function ClinicalModuleCard({ module, index = 0, className = "" }: { module: ClinicalModule; index?: number; className?: string }) {
  const Icon = iconMap[module.icon];

  return (
    <article className={`group relative min-h-[300px] overflow-hidden rounded-[2rem] border border-white/10 bg-white/[0.045] p-5 shadow-card backdrop-blur-xl transition duration-300 hover:-translate-y-2 hover:border-accent-soft/30 hover:bg-white/[0.075] hover:shadow-glow md:min-h-[360px] md:p-6 ${className}`}>
      ...
      <div className="relative flex h-full flex-col">
        ...
        <p className="mt-3 line-clamp-3 text-base leading-7 text-text-primary md:mt-4 md:text-lg">{module.emotionalLine}</p>
